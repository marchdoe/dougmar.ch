...
4. `daily-redesign.js` runs the agents. The Art Director decides the hero line, the composition, the chassis and the palette, and writes `preset.ts`. Before anything renders, code checks the reply against itself (`scripts/utils/ad-spec-checks.js`): every hex in the spec's Color Specification must be one the preset defines, `hero_scale` may not pass the biggest step on the chassis ramp, and `hero_step_360` must be a step on it. A mismatch sends the Art Director back once with the finding in the brief. The Mockup Designer renders one HTML mockup. Code measures it first (`scripts/utils/mockup-precheck.js`: canvas, colour and hero against the declared floors, the mark's presence, size, colour mode and lockup, and horizontal scroll and cut text on the phone), and the Mockup Critic judges what a measurement cannot see from the screenshots. Either can send it back; a revision returns FIND/REPLACE edits to the page rather than a new page (`scripts/utils/mockup-patch.js`), and a round whose measured faults did not move ends the loop. The React Engineer translates the approved mockup into the routes and components.
5. Gates. The build must pass `pnpm build`, the token gate (no unresolved Panda tokens), the static checks (biome, tsc, and `fallow audit --base HEAD`, the same audit CI's architecture job runs), and the surface gate, which measures every route at 360, 820 and 1440 in both colour schemes and fails on horizontal overflow and cut-off text. It also measures the share card (`/og`) once at 1200x630, the size the archive step captures it at, and fails a card whose text or brand lockup runs past its edges or into the 40px safe margin. Then the Screenshot Critic sees the home page at 1440 in both schemes, phone renders of the home page, `/about` and a project page, and a project page at 1440.
