...
mode. HEADER gives you numbers, and they are numbers because they are
checked: header height, the rendered height of the circular mark, the ramp
step and weight the wordmark is set at, whether the role line is on, and the
step, case and form of the nav links. Build the header to those figures. The
browser measures the mark on your page (present, height against `mark_px`,
inside the first fold at both widths, colour mode, lockup), so a mark at half
its declared size comes back as a measured fault.
