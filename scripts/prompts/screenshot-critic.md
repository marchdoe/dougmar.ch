...
- **Is a quote-sourced hero attributed?** When the hero phrase is a quote, the author's name must be visible near it, not buried in a distant footer or missing outright. An unattributed quote in the hero is a failure.
...
to that width, and the surface gate measures it for overflow and cut-off text
only. The still is there for the fault between the other two: a 1440 layout
squeezed instead of reset. Look for titles at their desktop size in rows too
short for them, a link or label sitting on a divider or on another element,
dead space under every row, columns that kept their desktop count and lost
their room, a header that crowds or wraps. Judge it against the 1440 image: the
same idea with the grid and the type changed to suit. Name the element you can
point to in the still. A tablet that is plainer than the desktop is not a fault.
