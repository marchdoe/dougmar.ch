/**
 * The archive phase (#221): the night shipped. Capture the og card, refresh
 * __root.tsx's og:image against what the capture left on disk, write the
 * archetype, and archive the build with every artifact the run collected.
 * Leaves the run's return value on `state.result` and sets
 * `state.archiveRan`, after which nothing rolls back.
 */
 ...
 */
async function captureOgCard(date, { root, writtenPaths }) {
  try {
 ...
 */
async function writeArchetype(date, archetype, { root }) {
  if (!archetype) return
  try {
 ...
 * from an empty file.
 *
 * Pulled out of the archive epilogue so the assembly is a pure function with a
 ...
 * before the og capture ran, so its og:image named today's PNG on the
 * strength of a capture that hadn't happened yet. Refresh it now that the
 * capture has had its one chance: buildOgMetaEntries checks disk this time,
 * so a failed capture ships default.png instead of a 404 (#399).
 * Non-blocking.
 * @param {import('./run-state.js').RunState} state
 */
async function refreshRootOgImage(state) {
  const { root, today } = state
  const { result: artDirectorResult, chosenChassis } = state.ad
  try {
...
/**
 * The per-build artifacts, gathered from the state.
 * @param {import('./run-state.js').RunState} state
 */
function artifactsOf(state) {
  const { ad, design } = state
  return archiveArtifacts({
    finalScreenshot: state.finalScreenshot,
    mockup: design.mockup,
    mockupScreenshot: design.mockupScreenshot,
    verdicts: state.verdicts,
    shellDecl: ad.shellDecl,
    headerDecl: ad.headerDecl,
    typeDecl: ad.typeDecl,
    mobileDecl: ad.mobileDecl,
    motionDecl: ad.motionDecl,
    heroSource: ad.result.heroSource,
    chosenComposition: ad.chosenComposition,
    chosenLane: design.chosenLane,
    measurablesDecl: ad.measurablesDecl,
...
/**
 * The success epilogue for the first-pass and the repaired build alike:
 * archive artifacts, persist the archetype, shape the return value. The
 * two differ only in the rationale suffix.
 * @param {import('./run-state.js').RunState} state
 */
export async function runArchivePhase(state) {
  const { root, today, signals, weights } = state
  const { tokenResult, chosenArchetype } = state.ad
  const rationaleSuffix = state.rationaleSuffix
  await captureOgCard(today, { root, writtenPaths: state.writtenPaths })
  await refreshRootOgImage(state)

  const allFiles = [...tokenResult.files, ...state.engineer.result.files]
  ...
  state.result = { rationale, design_brief: designBrief, files: allFiles }
}
